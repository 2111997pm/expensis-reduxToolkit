    ...
    if (expense.editItems !== null) {
      ...
      try {
        const res = await axios.get(
          `https://expensis-8cd9b-default-rtdb.firebaseio.com/${email}/expenses.json`
        );
        ...
        try {
          const res = await axios.delete(
            `https://expensis-8cd9b-default-rtdb.firebaseio.com/${email}/expenses/${Id}.json`
          );
        } catch (error) {
          alert(error);
        }
    ...
    <div className={classes.body}>
      <form onSubmit={submitHandler} ref={formRef}>
        ...
        <div>
          ...
          <Button variant="danger" onClick={hideExpenseHandler}>
            Cancel
          </Button>
        </div>
      </form>
