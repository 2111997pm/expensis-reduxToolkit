...
const ExpenseList = () => {
  ...
  const dltClickHandler = async (item) => {
    ...
    try {
      const res = await axios.get(`https://expensis-8cd9b-default-rtdb.firebaseio.com/${email}/expenses.json`)

      const data = res.data;
      const Id = Object.keys(data).find((eleId) => data[eleId].id === item.id);
      try {
        const res = await axios.delete(`https://expensis-8cd9b-default-rtdb.firebaseio.com/${email}/expenses/${Id}.json`)
      } catch (error) {
        alert(error)
      }
    } catch (error) {
  ...
  const clickActPremiumHandler = async () => {
    ...
    try {
      const res = await axios.post(
        `https://expensis-8cd9b-default-rtdb.firebaseio.com/${email}/userDetail.json`, {isPremium: true}
      );
    } catch (error){ 
      alert(error)
    }
